...
public class ApiCaseController extends BaseController{
	...
	@ResponseBody
	public Map<String, Object> runCase(HttpServletRequest request, @RequestParam("api-case-run-id") String runid, @RequestParam("api-case-run-version") String version, @RequestParam("api-case-run-account") String account) {
		try {
			runService.run(ApiRunType.CASE, Integer.parseInt(runid), Integer.parseInt(account), Integer.parseInt(version), getCurrentUserName(request));
			return successJson();
		} catch (Exception e) {
	...
	public Map<String, Object> createOrUpdate(@RequestParam("api-case-id") String id, @RequestParam("api-case-interface") String inter, 
			@RequestParam("api-case-name") String name, @RequestParam("api-case-strategy") String strategy, 
			@RequestParam("api-case-body") String body, @RequestParam("api-case-run") String run) {
		try {
			if(id == null || id.isEmpty()){
				Integer pid = caseService.create(new ACase(new AInterface(Integer.parseInt(inter)), name.trim(), jsonFormat(body, false), strategy.trim(), Integer.parseInt(run)));
				if(pid != null){
					return successJson();
				}else{
				...
				ACase aCase = caseService.update(new ACase(Integer.parseInt(id), new AInterface(Integer.parseInt(inter)), name.trim(), jsonFormat(body, false), strategy.trim(), Integer.parseInt(run)));
				if(aCase != null){
					return successJson();
				}else{

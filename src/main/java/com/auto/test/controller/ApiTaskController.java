...
public class ApiTaskController extends BaseController{
	...
	public Map<String, Object> createOrUpdate(HttpServletRequest request, @RequestParam("api-task-id") String id, @RequestParam("api-task-run") String run,
			...
			@RequestParam("api-task-email-hide") String email, @RequestParam("api-task-compare") String compare, @RequestParam("api-task-platform") String platform) {
		try {
			email = "1".equals(mail) ? email : null;
			if(isNull(id)){
				Integer tid = apiTaskService.create(new ATask(project, version, account, Integer.parseInt(run), Integer.parseInt(monitor), time, getCurrentUserName(request), Integer.parseInt(compare), Integer.parseInt(platform), Integer.parseInt(mail), trimArray(email)));
				if(tid != null){
				...
				ATask aTask = apiTaskService.update(new ATask(Integer.parseInt(id), project, version, account, Integer.parseInt(run), Integer.parseInt(monitor), time, getCurrentUserName(request), Integer.parseInt(compare), Integer.parseInt(platform), Integer.parseInt(mail), trimArray(email)));
				if(aTask != null){

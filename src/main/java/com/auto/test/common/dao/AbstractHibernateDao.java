...
public abstract class AbstractHibernateDao <T extends java.io.Serializable> implements IBaseDao<T> {
	...
	public Integer findMaxCount() {
		Object obj = getCurrentSession().createQuery("select max(id) from " + clazz.getName()).uniqueResult();
		return obj == null ? 0 : ((Integer) obj).intValue();
	}
	
	@Override
	public Integer findMaxCountBy(String field, String text) {
		Object obj = getCurrentSession().createQuery("select max(id) from " + clazz.getName() + " where " + field + " = " + text).uniqueResult();
		return obj == null ? 0 : ((Integer) obj).intValue();
	}

	@Override
	public Integer findMinCount() {
		Object obj = getCurrentSession().createQuery("select min(id) from " + clazz.getName()).uniqueResult();
		return obj == null ? 0 : ((Integer) obj).intValue();
	}
	...
	@Override
	public final List<T> findAllOrderAsc() {
		return getCurrentSession().createQuery("from " + clazz.getName() + " order by id asc").list();
	}
